...
from judgesync.types import ScoreRange, EvaluationItem
...
class TestDataLoader:
    ...
    def test_load_from_csv_with_metadata(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as f:
            writer = csv.writer(f)
            writer.writerow(['question', 'response', 'human_score', 'category', 'difficulty'])
        ...
        try:
            ...
            assert len(loader) == 1
            assert loader.items[0].metadata == {'category': 'math', 'difficulty': 'easy'}
        finally:
