...
from judgesync.types import ScoreRange, EvaluationItem, AlignmentResults
...
class TestAlignmentTracker:
    ...
    def test_init_with_system_prompt(self):
        ...
        with patch('judgesync.alignment.Judge') as mock_judge_class:
            tracker = AlignmentTracker(
                score_range=ScoreRange.PERCENTAGE,
    ...
    def test_get_best_prompt_with_history(self, mock_judge_class):
        ...
        tracker.history = [
            {
                "prompt": "Okay prompt",
                "results": AlignmentResults(kappa_score=0.5, agreement_rate=0.6, sample_size=10),
                "items_count": 10
            },
            {
                "prompt": "Best prompt",
                "results": AlignmentResults(kappa_score=0.9, agreement_rate=0.95, sample_size=10),
                "items_count": 10
            },
            {
                "prompt": "Bad prompt",
                "results": AlignmentResults(kappa_score=0.2, agreement_rate=0.3, sample_size=10),
                "items_count": 10
            }
        ]
    ...
    def test_summary(self, mock_judge_class):
        ...
        tracker.history = [{
            "prompt": "Test",
            "results": AlignmentResults(kappa_score=0.75, agreement_rate=0.85, sample_size=2),
            "items_count": 2
        }]
